...
public class AdminController {
    ...
    public ResponseEntity<DefaultResponse> updateBooking(Authentication authentication, @Valid @RequestBody UpdateBookingRequest request) {
        ...
        bookingService.updateBooking((Integer)authentication.getPrincipal(), request, userOptional);
        return ResponseEntity.ok(new DefaultResponse(HttpStatus.OK, "OK"));
    }

    @GetMapping("bookings/get/{floor}")
    public ResponseEntity<AdminBookingsFloorResponse> getFloorBookings(Authentication authentication, @PathVariable Integer floor) {
        log.info("Запрос на получение слотов этажа = {} от администратора userId={}", floor, (Integer)authentication.getPrincipal());
        List<AdminBookingDto> bookings = bookingService.getAdminBookingsByFloor(floor, (Integer)authentication.getPrincipal());
